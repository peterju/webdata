...
urls = [f"{base_url}{i:04d}/" for i in range(1, 484)]
...
async def generate_html_pages(results_dict: dict[str, list[str]], items_per_page: int = 5):
    ...
    for page_idx in range(page_count):
        ...
        for idx, (key, value) in enumerate(list(results_dict.items())[start_idx:end_idx], start=1):
            ...
            html_content += '</p>'
        html_content += "\n</body>\n</html>"
        with open(f"html_result/guanghai_{page_idx + 1}.html", "w", encoding="utf-8") as file:
    ...
    index_content += "\n</ul>\n</body>\n</html>"
    with open("html_result/guanghai.html", "w", encoding="utf-8") as file:
...
async def generate_text_files(results_dict: dict[str, list[str]], items_per_file: int):
    ...
    for idx, (key, value) in enumerate(results_dict.items(), start=1):
        ...
        if (idx - 1) % items_per_file == 0:
            ...
            with open(f"text_result/guanghai_{file_idx}.txt", "w", encoding="utf-8") as file:
                file.write(f"第 {idx:04d} 講：{key}\n")
        else:
            with open(f"text_result/guanghai_{file_idx}.txt", "a", encoding="utf-8") as file:
                file.write(f"\n第 {idx:04d} 講：{key}\n")
        # 將換行符號消除，再寫入文件
